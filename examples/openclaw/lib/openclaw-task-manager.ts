...
const RESEARCH_PATTERN = /\b(research|analy[sz]e|summary|summarize|news|search|look up|investigate)\b/i;
...
export class OpenClawTaskManager {
	...
	constructor(private readonly opts: OpenClawTaskManagerOptions) {
		...
		this.createThreadId = opts.createThreadId ?? (() => randomUUID().replaceAll('-', '').slice(0, 12));
	}
	...
	async acquire(request: OpenClawTaskAcquireRequest): Promise<OpenClawTaskLease> {
		...
		const semaphore = await this.acquireSemaphore(taskId, thread.threadId, domain, operation, request.onQueued);
	...
	getStats(): { activeTasks: number; activeSlots: number; queuedTasks: number; activeThreads: number } {
		return {
